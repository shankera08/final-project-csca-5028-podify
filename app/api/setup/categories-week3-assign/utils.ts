...
export class Utils {
    ...
    static async createCategoryTable(): Promise<boolean> {
        const result =
            await sql.query(`
                CREATE TABLE IF NOT EXISTS category_week3 ( id SERIAL PRIMARY KEY, category_id INT UNIQUE, name VARCHAR(255) );
            `);
        return !!result;
    }
    ...
    static async upsertCategories(categoryList: ICategory[]): Promise<boolean> {
        ...
        const query = `INSERT INTO category_week3 (category_id, name) VALUES ${valuesStr} ON CONFLICT (category_id) DO NOTHING;`;
